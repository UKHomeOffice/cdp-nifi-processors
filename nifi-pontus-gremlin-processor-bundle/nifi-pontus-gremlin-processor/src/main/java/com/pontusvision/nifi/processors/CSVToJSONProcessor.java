 ...
 */
@Tags({"JSON", "CSV"})
@CapabilityDescription("Convert CSV to JSON.")
public class CSVToJSONProcessor extends AbstractProcessor {
  ...
  final static PropertyDescriptor CSV_USE_FIRST_LINE_AS_HEADERS = new PropertyDescriptor.Builder()
    .name("Use first line as headers")
    .defaultValue("true")
    .required(true)
  ...
  final static PropertyDescriptor CSV_MAX_NUM_BATCHED_ENTRIES = new PropertyDescriptor.Builder()
    .name("The number of batched entries")
    .defaultValue("1")
    .required(true)
  ...
  final static PropertyDescriptor CSV_HEADERS = new PropertyDescriptor.Builder()
    .name("CSV Headers")
    .required(false)
    .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
    .build();

  final static PropertyDescriptor CSV_SEPARATOR = new PropertyDescriptor.Builder()
    .name("CSV Separator")
    .defaultValue(",")
    .required(true)
    .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
  ...
  public static final Relationship FAILURE = new Relationship.Builder()
    ...
    .build();
  public static final Relationship NEED_MORE = new Relationship.Builder()
    .name("NEED_MORE")
    .description("Need More relationship, used if the header is being read and we need more data to process the next record")
    .build();
  ...
  public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
    ...
    if (separatorChar == null) {
      ...
      safeCSVSplitPattern = Pattern.compile(String.format("%s(?=([^\"]*\"[^\"]*\")*[^\"]*$)",
    ...
    if (csvHeaders == null) {
      ...
      if (!csvHeadersUseFirstLineAsVal.asBoolean() && !csvHeadersPropVal.isSet()) {
        flowfile = session.putAttribute(flowfile, "error", "Must either set the option 'Use first line as headers' to true or set the option 'CSV Headers' to a comma-separated string of values.");
...
//              sb.setLength(0);
            ...
            if (csvSplit.length != csvHeaders.length) {
              flowfile = session.putAttribute(flowfile, "error", "Number of columns doesn't match the number of items in the row.");
