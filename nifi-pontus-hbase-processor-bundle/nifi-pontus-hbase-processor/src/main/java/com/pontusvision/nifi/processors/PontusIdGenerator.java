...
@Tags({"pontus", "POLE", "GUID", "UUID", "HBASE"})
@CapabilityDescription("Gets the incoming object, checks HBASE to see whether there's a matching record, and if there " +
        "is, returns its GUID; if there isn't, it creates a new GUID.")
public class PontusIdGenerator extends AbstractProcessor {
    ...
    final PropertyDescriptor HBASE_CLIENT_SERVICE = new PropertyDescriptor.Builder()
...
//    static final Pattern COLUMNS_PATTERN = Pattern.compile("\\w+(:\\w+)?(?:,\\w+(:\\w+)?)*");
    ...
    final PropertyDescriptor TABLE_NAME = new PropertyDescriptor.Builder()
            .name("Table Name")
...
//            .build();
    ...
    final PropertyDescriptor FILTER_EXPRESSION = new PropertyDescriptor.Builder()
            ...
            .description("An HBASE Filter String with replaceable tokens using ~~{attrib}~~  in the format of " +
...
//        }
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        final String startingFilter = StringReplacer.replaceTokensEscapeSingleQuotes(replacementMap, filterExpression, 5, "\\");
        ...
        try {
            ...
            if (numMatches == 1) {
                ...
                attributes.put("pontus.id.uuid", rowKeyString);
                attributes.put("pontus.id.type", UUIDType.NEW.toString());
                attributes.put("pontus.match.status", (numMatches > 1) ? MatchStatus.MULTIPLE.toString() : MatchStatus.NO_MATCH.toString());
            ...
            if (putObjWithGUID){
                ...
                for (Map.Entry<String,String> entry : putAttribsEntrySet ) {
                    ...
                    if (delim >= 0){
                       PutColumn col = new PutColumn(key.substring(0,delim).getBytes(), key.substring(delim+1).getBytes(),entry.getValue().getBytes());
