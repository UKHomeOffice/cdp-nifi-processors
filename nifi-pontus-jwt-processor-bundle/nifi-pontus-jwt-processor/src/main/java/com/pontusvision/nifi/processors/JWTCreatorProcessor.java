 ...
 * @author Leo Martins
 */
@Tags({"pontus","JWT", "Creation", "JSON Web Tokens"})
@CapabilityDescription("Create JWT Tokens.")
public class JWTCreatorProcessor extends AbstractProcessor {
...
//          .compact();
  ...
  public static final PropertyDescriptor SSL_CONTEXT = new PropertyDescriptor.Builder()
    ...
    .description("The TLS Context Service to use in order to sign the JWTs created")
    .required(true)
    .identifiesControllerService(SSLContextService.class)
    .build();

  public static final PropertyDescriptor JWT_KEY_ALGO = new PropertyDescriptor.Builder()
    .name("JWT Key Algo")
    .description("The encryption keyAlgo for the key in the key store of the TLS context passed through the option above. ")
    .defaultValue(JWSAlgorithm.RS512.toString())
    .required(true)
    .expressionLanguageSupported(false)
    .addValidator(keySignValidator)
    .allowableValues(getAlgos())
    .build();


  public static final PropertyDescriptor JWT_ATTRIBUTE_NAME = new PropertyDescriptor.Builder()
    .name("JWT Attribute Name")
    .description("If present, the attribute name where the JWT will be sent to (if creating), or read from (if decoding). ")
    .defaultValue("")
    .required(true)
    .expressionLanguageSupported(false)
  ...
  public static final PropertyDescriptor JWT_TIME_TO_LIVE_MS = new PropertyDescriptor.Builder()
    .name("JWT time to live")
    .description("The JWT time to live in milliseconds; use -1 for unlimited. ")
    .defaultValue("-1")
    .required(true)
    .expressionLanguageSupported(false)
...
//    session.transfer(flowfile, SUCCESS);
  ...
  public static Key getPublicKey(SSLContextService sslService, String alias) throws IOException, KeyStoreException, CertificateException, NoSuchAlgorithmException, UnrecoverableKeyException {
...
//      new KeyPair(publicKey, (PrivateKey) key);
  ...
  public static Key getPrivateKey(SSLContextService sslService, String alias) throws IOException, KeyStoreException, CertificateException, NoSuchAlgorithmException, UnrecoverableKeyException {
    FileInputStream is = new FileInputStream(sslService.getKeyStoreFile());
